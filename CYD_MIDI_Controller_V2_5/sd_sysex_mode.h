...
void sysexRefreshList() {
  sysexFileCount = sdListFiles(SYSX_DIR, sysexFiles, SYSX_MAX_FILES);
...
void sysexSaveToSD(const char* name) {
  ...
  SD.mkdir(SYSX_DIR);
  String path = String(SYSX_DIR) + "/" + name + ".syx";
...
void sysexDeleteFile(const String& filename) {
  if (!sdMounted) return;
  SD.remove((String(SYSX_DIR) + "/" + filename).c_str());
...
bool sysexLoadForSend(const String& filename) {
  ...
  String path = String(SYSX_DIR) + "/" + filename;
...
void drawSDSysexMode() {
  ...
  tft.setTextColor(THEME_TEXT_DIM, THEME_BG);
  tft.drawString("FILES IN /SYSX/", 8, 92, 1);
...
void handleSDSysexMode() {
  ...
  for (int i = 0; i < SYSX_VIS_ROWS; i++) {
    if (isButtonPressed(4, 102 + i * FILELIST_ROW_H, 215, FILELIST_ROW_H - 2)) {
      ...
      if (idx < sysexFileCount) { sysexSelected = idx; drawSDSysexMode(); }
      return;
    }
  }
  ...
  if (isButtonPressed(271, 102, 45, 25) && sysexFileCount > 0) {
    sdInit();
    if (sysexLoadForSend(sysexFiles[sysexSelected])) {
      sysexSending    = true;
  ...
  if (isButtonPressed(271, 130, 45, 25) && sysexFileCount > 0) {
    sysexDeleteFile(sysexFiles[sysexSelected]);
    drawSDSysexMode(); return;
  }
}
