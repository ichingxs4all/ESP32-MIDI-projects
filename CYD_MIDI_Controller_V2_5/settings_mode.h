...
void drawSettingsMode() {
  ...
  tft.setTextColor(THEME_TEXT_DIM, THEME_BG);
  tft.drawString("Beat mapping", 115, 212, 1);

#ifdef ENABLE_WIFI
  drawRoundButton(170, 200, 90, 35, "WIFI", THEME_ACCENT);
  tft.setTextColor(THEME_TEXT_DIM, THEME_BG);
  tft.drawString("AppleMIDI", 275, 212, 1);
#endif
}
...
#ifdef ENABLE_WIFI
  if (isButtonPressed(170, 200, 90, 35)) { inWifiSubMenu = true; drawWifiSettingsMode(); return; }
#endif
}
...
void initEeprom() {
  ...
  // WiFi: blank SSID, password, flags
