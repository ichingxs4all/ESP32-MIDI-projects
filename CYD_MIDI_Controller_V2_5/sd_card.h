...
bool sdInit() {
  ...
  // CYD touchscreen uses VSPI (pins 25/39/32/33).
  // SD card must use HSPI to avoid reconfiguring that bus.
  // CYD SD card slot is wired to: SCK=14... no — the CYD SD slot uses
  // its own dedicated pins: SCK=18, MISO=19, MOSI=23, CS=5 on HSPI.
  ...
  sdCreateDirs();   // ensure standard directories exist
  return true;
}
