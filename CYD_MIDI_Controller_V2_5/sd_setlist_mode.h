...
void sdSetRefreshFiles() {
  sdSetFileCount = sdListFiles(SDSET_DIR, sdSetFiles, SDSET_MAX_FILES);
}

void sdSetLoad(const String& filename) {
  if (!sdMounted) { sdSetStatus = "No SD card"; return; }
  String path = String(SDSET_DIR) + "/" + filename;
...
void drawSDSetlistMode() {
  ...
  if (!sdSetLoaded) {
    ...
    for (int i = 0; i < SDSET_FILES_VIS; i++) {
      ...
      tft.fillRect(4, rowY, 255, 27, sel ? THEME_PRIMARY : THEME_SURFACE);
      tft.setTextColor(sel ? THEME_BG : THEME_TEXT, sel ? THEME_PRIMARY : THEME_SURFACE);
      if (idx < sdSetFileCount) tft.drawString(sdSetFiles[idx], 8, rowY + 7, 2);
    }
...
void handleSDSetlistMode() {
  ...
  if (!sdSetLoaded) {
    ...
    if (isButtonPressed(265, 120, 50, 27)) {
      sdInit();
      if (sdSetFileCount > 0) sdSetLoad(sdSetFiles[sdSetFileSelected]);
      drawSDSetlistMode(); return;
    }
    for (int i = 0; i < SDSET_FILES_VIS; i++) {
      if (isButtonPressed(4, 55 + i * 30, 255, 27)) {
        sdSetFileSelected = sdSetFileScrollTop + i;
        drawSDSetlistMode(); return;
      }
    }
  } else {
