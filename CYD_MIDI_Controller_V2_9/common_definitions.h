#ifndef COMMON_DEFINITIONS_H
#define COMMON_DEFINITIONS_H

#include <TFT_eSPI.h>
#include <XPT2046_Touchscreen.h>
#include <BLEDevice.h>

int channel = 1; //MIDI channel to send to
int  gateLength = 30; //Gate length ; time between note-on and note-off

// Color scheme
#define THEME_BG         0x0841
#define THEME_SURFACE    0x2945
#define THEME_PRIMARY    0x06FF
#define THEME_SECONDARY  0xFD20
#define THEME_ACCENT     0x07FF
#define THEME_SUCCESS    0x07E0
#define THEME_WARNING    0xFFE0
#define THEME_ERROR      0xF800
#define THEME_TEXT       0xFFFF
#define THEME_TEXT_DIM   0x8410

// BLE MIDI UUIDs
#define SERVICE_UUID        "03b80e5a-ede8-4b33-a751-6ce34ec4c700"
#define CHARACTERISTIC_UUID "7772e5db-3868-4112-a1a9-f2669d106bf3"

// Touch handling
struct TouchState {
  bool wasPressed = false;
  bool isPressed = false;
  bool justPressed = false;
  bool justReleased = false;
  int x = 0, y = 0;
};

// App modes
enum AppMode {
  MENU,
  KEYBOARD,
  SEQUENCER,
  BOUNCING_BALL,
  PHYSICS_DROP,
  RANDOM_GENERATOR,
  XY_PAD,
  ARPEGGIATOR,
  GRID_PIANO,
  AUTO_CHORD,
  LFO,
  SETTINGS,
  MONITOR,
  MIDI_CLOCK_MODE,
  PGMCHANGE_MODE,
  SD_MENU_MODE,
  SD_SEQ_MODE,
  SD_PLAYER_MODE,
  SD_RECORDER_MODE,
  SD_SETLIST_MODE,
  SD_INFO_MODE,
  SD_SYSEX_MODE,
  SD_FILETRANSFER_MODE,
  SLIDER_MODE
};

// Music theory
struct Scale {
  String name;
  int intervals[12];
  int numNotes;
};

// Global scale definitions
extern Scale scales[];
extern const int NUM_SCALES;

// Global objects - declared in main file
extern TFT_eSPI tft;
extern XPT2046_Touchscreen ts;
extern BLECharacteristic *pCharacteristic;
extern bool deviceConnected;
extern uint8_t midiPacket[];
extern TouchState touch;
extern AppMode currentMode;

// Forward declarations — defined in the main .ino, needed by mode headers
void enterMode(AppMode mode);
void exitToMenu();
void drawMenu();

// Forward declarations — wifi_settings_mode.h (included after settings_mode.h)
#ifdef ENABLE_WIFI
void drawWifiSettingsMode();
void handleWifiSettingsMode();
void initializeWifiSettingsMode();
void storeWifiSettings();
void loadWifiSettings();
void wifiConnect();
void wifiConnectStart();
void wifiDisconnect();
#endif

// Forward declarations — mode consumer functions called by early-included headers
void monitorPushEvent(byte status, byte data1, byte data2);
void clockReceiveByte(byte b);
void sysexReceiveByte(byte b);

// Forward declarations — CV output module (cv_output.h included after midi_utils.h)
extern bool cvEnabled;
void cvProcessMIDI(byte status, byte data1, byte data2);
void drawCVSettingsMode();
void handleCVSettingsMode();
void cvSaveSettings();
void cvLoadSettings();
void initializeCVOutput();

#endif