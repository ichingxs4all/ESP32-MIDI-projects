...
static void ftDoList(const char* path) {
  ...
  while (entry) {
    // entry.name() returns full path on ESP32 SD library
    if (entry.isDirectory()) {
      snprintf(buf, sizeof(buf), "FT:D:%s", entry.name());
    } else {
      snprintf(buf, sizeof(buf), "FT:F:%s:%u", entry.name(), (unsigned)entry.size());
    }
    ftSendLn(buf);
    entry.close();
