...
struct DropBall {
  ...
  unsigned long noteOffTime;  // when to send note-off (0 = nothing pending)
  int note;
  String noteName;
};
...
void initializePhysicsDropMode() {
  ...
  platforms[0] = {80, 160, 60, 8, 0.2, THEME_PRIMARY, false, 60, "C4", 0, 0};
  platforms[1] = {180, 140, 50, 8, -0.3, THEME_SECONDARY, false, 64, "E4", 0, 0};
  platforms[2] = {120, 120, 40, 8, 0.1, THEME_ACCENT, false, 67, "G4", 0, 0};
  numPlatforms = 3;
}
...
void updatePhysics() {
  ...
  // Drain any pending note-offs (balls and platforms)
  ...
  for (int i = 0; i < numPlatforms; i++) {
    if (platforms[i].noteOffTime != 0 && now >= platforms[i].noteOffTime) {
      sendMIDI(0x80, platforms[i].note, 0);
  ...
  for (int i = 0; i < MAX_DROP_BALLS; i++) {
    ...
    if (dropBalls[i].y + dropBalls[i].size >= 175) {
      ...
      if (abs(dropBalls[i].vy) > 1) {
        ...
        dropBalls[i].noteOffTime = millis() + gateLength;  // schedule non-blocking note-off
      }
    }
  }
    ...
    for (int p = 0; p < numPlatforms; p++) {
      ...
      if (dropBalls[b].x + dropBalls[b].size >= platforms[p].x &&
        ...
        if (!platforms[p].active) {
          ...
          platforms[p].noteOffTime = millis() + gateLength;  // schedule non-blocking note-off
