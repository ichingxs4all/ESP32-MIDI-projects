...
int ballScale = 3;  // Scale selection
    ...
    for (int w = 0; w < NUM_WALLS; w++) {
      ...
      if (collision) {
        if (deviceConnected) {
          ...
          walls[w].noteOffTime = millis() + gateLength;  // schedule non-blocking note-off
        }
